...
public class Crudoperation {
	...
	public static void updatedata(Integer Enroll) {
		String u = "update students set Name = 'Abid' , Stream = 'Arts' where Enroll = '"+Enroll+ " '";
		try {
			int s = st.executeUpdate(u);
			if(s>0) {
	...
	public static void searchdata(String Name) {
		String ss = "select * from students where  name = '"+Name+"'";
		try {
			ResultSet j = st.executeQuery(ss);
			while(j.next()) {
